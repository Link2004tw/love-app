...
export async function joinCouple(inviteCode, uid) {
  ...
  const encryptionKey = coupleData.encryptionKey || generateKey();

  await coupleDoc.ref.update({
    memberIds: [...coupleData.memberIds, uid],
    ...(coupleData.encryptionKey ? {} : { encryptionKey }),
  });

  await adminDb.collection("users").doc(uid).update({ encryptionKey });

  return { id: coupleDoc.id, ...coupleData, encryptionKey };
}
